...
сервиса, которую можно использовать для перехода по оригинальному URL. <br>
...
и автоматически откроется браузер для перенаправления на изначальный длинный URL. <br>
...
в выводе команды help). <br>

## Сервис реализует следующий функционал:

1.  При первом вводе URL для сокращения создаёт для пользователя уникальный UUID для идентификации.
...
1.  login определённый_UUID - идентификация по имеющемуся UUID
...
7.  stats URL_ссылки - просмотреть статистику по отдельной действующей короткой ссылке
8.  delete URL_ссылки - удалить созданную ранее короткую ссылку
9.  manage URL_ссылки set limit значение - изменить лимит использований короткой ссылки
10. manage URL_ссылки set original_url значение - изменить URL, на который ведёт короткая ссылка
11. manage URL_ссылки set ttl значение - изменить срок действия имеющейся короткой ссылки
...
15. ввод URL без команды и без идентификации - также создаёт новый UUID при сокращении URL
...
## Тестирование
В проект добавлено 33 интеграционных и юнит теста (src/test). Они покрывают более 50% кода и все слои. Ряд интеграционных тестов проверяет
результат прохождения через все этапы работы сервиса от начала и до конца в рамках обработки одного запроса, имитируя ввод пользователя.

...
public class RedirectCommandHandler implements CommandHandler {
  ...
  public ViewModel handle(String[] commandArgs, UUID currentUserUUID) {
    ...
    if (commandArgs.length != 1 || shortLinkURL.isBlank()) {
      return new ErrorViewModel("Вы не ввели команду или URL для сокращения. Предоставленный ввод не распознан. Введите help для помощи по сервису.");
    } else {
      try {
      ...
      } catch (OriginalLinkNotFoundException | IOException | InvalidShortLinkException | InvalidOriginalLinkException | UsagesLimitReachedException e) {
